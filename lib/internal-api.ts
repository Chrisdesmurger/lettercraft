...
const INTERNAL_API_SECRET = process.env.INTERNAL_API_SECRET || 'lettercraft-internal-secret-2025'
  ...
  try {
    ...
    switch (action) {
      case 'sync':
        ...
        return {
          success,
          message: success ? 'Synchronisation réussie' : 'Échec synchronisation'
        }

      case 'create':
        // Pour les créations rapides depuis l'inscription
        if (!data.email || !data.firstName || !data.lastName) {
          return { success: false, error: 'email, firstName et lastName requis' }
        }
        ...
        return {
          success: !!result,
          message: result ? 'Contact créé avec succès' : 'Échec création contact'
        }

      default:
        return { success: false, error: `Action interne non supportée: ${action}` }
    }
  } catch (error) {
