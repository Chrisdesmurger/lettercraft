...
function parseStandardLetter(content: string): ParsedLetterResponse {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inBody || (!greeting && i > 2)) {
      ...
      if (
        !line.toLowerCase().includes("salutations") &&
  ...
  if (!subject && bodyLines.length > 0) {
    ...
    if (
      firstParagraph.toLowerCase().includes("application for") ||
...
function cleanSection(content: string): string {
  ...
  let cleaned = content
    ...
    .replace(
      /^.*\d{1,2}\s+(janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre)\s+\d{4}.*$/gm,
      "",
    )
    // Remove company address blocks
 ...
 */
export function validateContentCleanliness(content: string): {
  isClean: boolean;
  issues: string[];
} {
  const issues: string[] = [];
  ...
  if (
    /\b(\+\d{1,3}[\s\-]?\d{1,4}[\s\-]?\d{1,4}[\s\-]?\d{1,9})\b/.test(content)
  ) {
  ...
  if (
    /(Warm regards|Sincerely|Best regards|Cordialement),?\s*$/.test(content)
  ) {
