...
export type ToneKey = 'professionnel' | 'chaleureux' | 'direct' | 'persuasif' | 'créatif' | 'concis' | 'personnalisé'
 ...
 */
export const TONE_GUIDELINES: Record<Exclude<ToneKey, 'personnalisé'>, ToneGuideline> = {
  professionnel: {
    key: 'professionnel',
    label: 'Professionnel',
    description: 'Style formel et structuré, idéal pour les candidatures traditionnelles',
    styleDirectives: 'Style formel, clair, structuré, factuel. Utilise un langage soutenu, des phrases bien construites et un vocabulaire précis. Évite les familiarités et maintient une distance respectueuse.',
  ...
  chaleureux: {
    ...
    styleDirectives: 'Ton cordial, positif, humain et engageant. Exprime de l\'enthousiasme authentique, utilise un langage accessible tout en restant respectueux. Montre de la personnalité sans tomber dans la familiarité.',
  ...
  direct: {
    ...
    styleDirectives: 'Formulations concises, orientées résultats. Va droit à l\'essentiel, utilise des phrases courtes et impactantes. Privilégie les faits concrets et les réalisations mesurables.',
    example: 'Mes 5 ans d\'expérience en développement web correspondent exactement à vos besoins...'
  },
  persuasif: {
    key: 'persuasif',
    label: 'Persuasif',
    description: 'Met l\'accent sur la valeur ajoutée et les bénéfices',
    styleDirectives: 'Mettre en avant bénéfices, preuves et motivation. Utilise des arguments convaincants, des exemples concrets de réussites. Montre clairement la valeur ajoutée apportée à l\'entreprise.',
    example: 'Mon expertise vous permettra d\'augmenter vos performances de 30%...'
  },
  créatif: {
    key: 'créatif',
    label: 'Créatif',
    description: 'Style original avec des expressions variées et une touche d\'originalité',
    styleDirectives: 'Expressions variées, métaphores légères, originalité. Utilise un vocabulaire riche et imagé tout en restant professionnel. Apporte une touche de créativité qui reflète la personnalité.',
    example: 'Comme un puzzle dont chaque pièce trouve sa place, mes compétences s\'articulent parfaitement avec vos objectifs...'
  },
  concis: {
    key: 'concis',
    label: 'Concis',
    description: 'Maximum d\'impact en minimum de mots',
    styleDirectives: 'Phrases courtes, suppression du superflu. Privilégie l\'efficacité, utilise des mots forts et précis. Chaque phrase doit avoir un impact, éliminer tout mot inutile.',
    example: 'Développeur expérimenté. Résultats prouvés. Disponible immédiatement.'
  }
}
 ...
 */
export function getToneDirectives(toneKey: ToneKey, customText?: string): string {
  if (toneKey === 'personnalisé') {
    if (!customText || customText.trim().length === 0) {
      throw new Error('Le texte personnalisé est requis pour le ton personnalisé')
    }
  ...
  if (!guideline) {
    console.warn(`Ton non reconnu: ${toneKey}, utilisation du ton professionnel par défaut`)
...
export function isValidToneKey(toneKey: string): toneKey is ToneKey {
  const validKeys: ToneKey[] = ['professionnel', 'chaleureux', 'direct', 'persuasif', 'créatif', 'concis', 'personnalisé']
 ...
 */
export function validateToneParams(toneKey: string, customText?: string): { 
  valid: boolean
  errors: string[] 
} {
  const errors: string[] = []
