...
class BrevoContactsService {
  ...
  async createOrUpdateContact(
    contactData: BrevoContact,
  ): Promise<BrevoContactResponse | null> {
    try {
      ...
      if (!response.ok) {
        ...
        if (
          response.status === 400 &&
          errorData.includes("Contact already exist")
        ) {
          console.log(
            `🔄 Contact existe déjà, mise à jour: ${contactData.email}`,
          );
          return await this.updateContact(contactData.email, contactData);
        }

        console.error(
          `❌ Erreur création contact Brevo:`,
          response.status,
          errorData,
        );
        return null;
      ...
      console.error(
        "❌ Erreur inattendue lors de la création du contact:",
        error,
      );
      return null;
    }
  ...
  async updateContact(
    email: string,
    contactData: Partial<BrevoContact>,
  ): Promise<BrevoContactResponse | null> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.text();
        console.error(
          `❌ Erreur mise à jour contact Brevo:`,
          response.status,
          errorData,
        );
        return null;
      ...
      console.error(
        "❌ Erreur inattendue lors de la mise à jour du contact:",
        error,
      );
      return null;
    }
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          ...
          const errorData = await response.text();
          console.error(
            `❌ Erreur récupération contact Brevo:`,
            response.status,
            errorData,
          );
        }
      ...
      console.error(
        "❌ Erreur inattendue lors de la récupération du contact:",
        error,
      );
      return null;
    }
  ...
  async deleteContact(email: string): Promise<boolean> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.text();
        console.error(
          `❌ Erreur suppression contact Brevo:`,
          response.status,
          errorData,
        );
        return false;
      ...
      console.error(
        "❌ Erreur inattendue lors de la suppression du contact:",
        error,
      );
      return false;
    }
  ...
  async addContactToList(email: string, listId: number): Promise<boolean> {
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}/contacts/lists/${listId}/contacts/add`,
        {
          method: "POST",
          headers: this.getHeaders(),
          body: JSON.stringify({
  ...
  async removeContactFromList(email: string, listId: number): Promise<boolean> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.text();
        console.error(
          `❌ Erreur suppression de la liste:`,
          response.status,
          errorData,
        );
        return false;
      ...
      console.error(
        "❌ Erreur inattendue lors de la suppression de la liste:",
        error,
      );
      return false;
    }
  ...
  async bulkSyncUsers(
    ...
    console.log(
      `📊 Synchronisation terminée: ${success} succès, ${failed} échecs`,
    );
  ...
  async updateContactLists(
    ...
    try {
      ...
      if (!currentContact) {
        console.log(
          `📭 Contact non trouvé pour mise à jour des listes: ${email}`,
        );
        return false;
      }
      ...
      if (result) {
        console.log(
          `✅ Listes mises à jour pour ${email}: [${newListIds.join(", ")}]`,
        );
        return true;
      } else {
  ...
  async syncAllContactLists(): Promise<{ updated: number; failed: number }> {
    try {
      ...
      const { data: profiles, error } = await supabaseAdmin.from(
        "user_profiles",
      ).select(`
          user_id,
          first_name,
      ...
      for (const profile of profiles) {
        try {
          ...
          if (!authUser?.user?.email) {
            console.warn(
              `⚠️ Email non trouvé pour l'utilisateur ${profile.user_id}`,
            );
            failed++;
            continue;
          }
          ...
          console.error(
            `❌ Erreur sync listes pour ${profile.user_id}:`,
            error,
          );
          failed++;
        }
      ...
      console.log(
        `📊 Synchronisation des listes terminée: ${updated} mis à jour, ${failed} échecs`,
      );
   ...
   */
  async createMissingContacts(): Promise<{
    created: number;
    ...
    try {
      ...
      console.log(
        `📊 Vérification de ${profiles.length} utilisateurs dans Brevo`,
      );
      ...
      for (const profile of profiles) {
        try {
          ...
          console.error(
            `❌ Erreur création contact pour ${profile.user_id}:`,
            error,
          );
          failed++;
        }
      ...
      console.error(
        "❌ Erreur lors de la création des contacts manquants:",
        error,
      );
      return { created: 0, already_exists: 0, failed: 0 };
    }
...
export async function bulkSyncUsersToBrevo(
  userIds: string[],
): Promise<{ success: number; failed: number }> {
...
export async function syncAllContactLists(): Promise<{
  updated: number;
  failed: number;
}> {
...
export async function createMissingContacts(): Promise<{
  created: number;
  already_exists: number;
  failed: number;
}> {
