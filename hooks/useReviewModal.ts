 ...
 */
export function useReviewModal(options: UseReviewModalOptions = {}): UseReviewModalResult {
  const { 
    onReviewSubmit, 
    autoShow = true,
    letterContentRef,
  ...
  const { hasReachedEnd, isAtEnd, scrollPercentage, reset: resetScrollDetection } = useScrollEndDetection({
  ...
  const wasDismissedRecently = useMemo(() => 
    (letterId: string): boolean => {
      ...
      const expiry = letterMemory.dismissedAt + (MEMORY_EXPIRY_HOURS * 60 * 60 * 1000)
      return Date.now() < expiry
    }, [getModalMemory])

  // Mark letter as dismissed
  const markAsDismissed = useCallback((letterId: string) => {
    const memory = getModalMemory()
    memory[letterId] = {
      dismissed: true,
  ...
  const showReviewModal = useCallback((letterId: string) => {
