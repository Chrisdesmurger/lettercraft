...
export const defaultQuestions: QuestionnaireQuestion[] = [
  {
    ...
    type: "textarea",
    placeholder:
      "Décrivez ce qui vous attire dans cette opportunité et pourquoi vous souhaitez rejoindre cette entreprise...",
    required: true,
  ...
  },
  {
    id: "skills_match",
    title:
      "Quelles compétences correspondent le mieux aux attentes de l'offre ?",
  ...
  },
  {
    id: "company_values",
    title:
      "Comment vos valeurs personnelles s'alignent-elles avec celles de l'entreprise ?",
    type: "textarea",
    placeholder:
      "Expliquez comment votre personnalité et vos valeurs peuvent apporter une valeur ajoutée à l'entreprise...",
    required: true,
    validation: (value: string) => {
  ...
  },
  {
    ...
    type: "textarea",
    placeholder:
      "Informations additionnelles que vous souhaitez inclure dans votre lettre (optionnel)...",
    required: false,
  },
];

export function useQuestionnaireFlow(
  questions: QuestionnaireQuestion[] = defaultQuestions,
) {
    ...
    if (typeof window !== "undefined") {
      ...
      if (saved) {
        try {
          ...
          const validCurrentQuestion = Math.min(
  ...
  const safeCurrentQuestionIndex =
    questions.length > 0
      ? Math.min(Math.max(0, state.currentQuestion), questions.length - 1)
      : 0;
  const currentQuestion =
    questions.length > 0
      ? questions[safeCurrentQuestionIndex] || questions[0]
      : null;
  ...
  useEffect(() => {
    ...
    const needsUpdate =
      Object.keys(defaultAnswers).length > 0 ||
    ...
    if (needsUpdate) {
      setState((prev) => ({
        ...
        currentQuestion: Math.min(
          Math.max(0, prev.currentQuestion),
          questions.length - 1,
        ),
