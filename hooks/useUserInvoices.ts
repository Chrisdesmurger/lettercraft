...
export function useUserInvoices(user: User | null): UseUserInvoicesReturn {
  ...
  const fetchInvoices = async () => {
    ...
    try {
      ...
      try {
        console.log(
          "🧾 [useUserInvoices] Trying secure function get_user_invoices...",
        );
        ...
        if (!result.error) {
          ...
          fetchError = null;
          console.log(
            "🧾 [useUserInvoices] Success with secure function:",
            data?.length,
            "invoices",
          );
        } else {
        ...
        try {
          ...
          if (!result.error) {
            ...
            fetchError = null;
            console.log(
              "🧾 [useUserInvoices] Success with view:",
              data?.length,
              "invoices",
            );
          } else {
          ...
          console.log(
            "🧾 [useUserInvoices] Direct table result:",
            data?.length,
            "invoices",
            fetchError,
          );
      ...
      console.log(
        "🧾 [useUserInvoices] Successfully loaded",
        data?.length || 0,
        "invoices",
      );
      setInvoices(data || []);
      ...
      setError(
        err instanceof Error ? err.message : t("subscription.invoice.error"),
      );
    } finally {
      setLoading(false);
    }
