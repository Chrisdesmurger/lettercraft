  ...
  const refreshQuota = useCallback(async () => {
    try {
      ...
      if (quotaData.first_generation_date) {
        ...
        if (now >= baseResetDate) {
          ...
          const monthsElapsed = Math.floor(
            (now.getTime() - baseResetDate.getTime()) /
              (30 * 24 * 60 * 60 * 1000),
          );
      ...
      setQuota({
        ...
        remaining_letters:
          currentQuota.max_letters - currentQuota.letters_generated,
        reset_date: nextResetDate
          ? nextResetDate.toISOString()
          : currentQuota.reset_date || "",
  ...
  const incrementLetterCount = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const shouldSendWarning =
        ...
        (newLettersGenerated === quota.max_letters - 2 ||
          newLettersGenerated === quota.max_letters - 1);
...
export function usePreGenerationQuotaCheck() {
  const { quota, checkCanGenerate, refreshQuota, incrementLetterCount } =
    useQuota();
  ...
  const executeWithQuotaCheck = useCallback(
    async (action: () => Promise<void> | void): Promise<boolean> => {
      ...
      try {
        ...
        console.error(
          "Erreur lors de l'exécution avec vérification de quota:",
          error,
        );
        return false;
      }
