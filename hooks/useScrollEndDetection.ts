...
export function useScrollEndDetection(
  ...
  const checkScrollPosition = useCallback(() => {
    ...
    const scrollTop = element?.current ? element.current.scrollTop : window.pageYOffset
    ...
    const percentage = maxScroll > 0 ? Math.min((scrollTop / maxScroll) * 100, 100) : 100
    ...
    if (atEnd && !hasReachedEnd) {
      ...
      timeoutRef.current = setTimeout(() => {
        ...
        if (timeSinceLastScroll >= delay - 100) { // Tolérance de 100ms
