...
export function useLetterGeneration() {
  ...
  const analyzeJobOffer = useCallback(
    async (jobOfferText: string, sourceUrl?: string) => {
      if (!user) {
  ...
  const submitQuestionnaire = useCallback(
    async (responses: QuestionnaireData) => {
      if (!user || !flow.jobOffer || !activeCV) {
        setFlow((prev) => ({
          ...prev,
          error: "Données manquantes pour le questionnaire",
        }));
        return null;
      }
      ...
      try {
        ...
        const { data: questionnaireResponse, error: insertError } =
          await supabase
            ...
            .insert({
              ...
              writing_tone: responses.writing_tone || {
  ...
  const generateLetter = useCallback(
    async (settings?: any, questionnaireResponseParam?: any) => {
      // Utiliser le paramètre passé ou celui du flow
      const questionnaireResponse =
        questionnaireResponseParam || flow.questionnaireResponse;

      if (!user || !flow.jobOffer || !questionnaireResponse || !activeCV) {
        setFlow((prev) => ({
          ...prev,
          error: "Données manquantes pour la génération",
        }));
        return;
      }
      ...
      const success = await executeWithQuotaCheck(async () => {
        ...
        const response = await fetch("/api/generate-personalized-letter", {
          ...
          body: JSON.stringify({
            ...
            settings: {
              language:
                questionnaireResponse.language || settings?.language || "fr",
              // Priorité : settings passés explicitement > données du questionnaire > défaut
              tone:
                settings?.toneKey ||
                settings?.tone ||
                questionnaireResponse.writing_tone?.toneKey ||
                "professionnel",
              toneKey:
                settings?.toneKey ||
                questionnaireResponse.writing_tone?.toneKey ||
              ...
              customTone:
                settings?.customTone ||
                questionnaireResponse.writing_tone?.customText ||
                "",
        ...
        const { data: generatedLetter, error: insertError } = await supabase
          ...
          .insert({
            ...
            tone_key: settings?.toneKey || "professionnel",
            tone_custom: settings?.customTone || null,
          })
          .select()
          .single();
