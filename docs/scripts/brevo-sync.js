...
async function getContact(email) {
  try {
    ...
    const response = await fetch(
      `${BASE_URL}/api/sync-contact?email=${encodeURIComponent(email)}`,
    );
    const result = await response.json();
    ...
    console.log(
      "   📊 Attributs:",
      JSON.stringify(result.contact.attributes, null, 2),
    );
  ...
  "create-contact": {
    ...
    handler: (email, firstName, lastName, language = "fr") => {
      if (!email || !firstName || !lastName) {
        console.error(
          "❌ Usage: create-contact <email> <firstName> <lastName> [language]",
        );
        return false;
      }
...
function showHelp() {
  ...
  console.log(
    "  node scripts/brevo-sync.js create-contact user@test.com Jean Dupont fr",
  );
...
async function main() {
  ...
  if (!actions[action]) {
    ...
    console.error(
      '   Utilisez "help" pour voir la liste des actions disponibles',
    );
    process.exit(1);
  }
