...
async function testBrevoConnection() {
  try {
    ...
    console.log(
      "  BREVO_SENDER_EMAIL:",
      process.env.BREVO_SENDER_EMAIL || "NON DÉFINIE",
    );
    console.log(
      "  BREVO_SENDER_NAME:",
      process.env.BREVO_SENDER_NAME || "NON DÉFINIE",
    );

    // Vérifier la présence de l'API key
    if (!process.env.BREVO_API_KEY) {
      console.error(
        "❌ BREVO_API_KEY non définie dans les variables d'environnement",
      );
      console.log(
        "💡 Ajoutez BREVO_API_KEY=xkeysib-votre-clé dans votre .env.local",
      );
      console.log(
    ...
    if (!process.env.BREVO_API_KEY.startsWith("xkeysib-")) {
      console.error(
        '❌ BREVO_API_KEY semble invalide (doit commencer par "xkeysib-")',
      );
      return;
    }
