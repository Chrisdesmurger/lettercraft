...
async function processDeletions() {
  console.log(
    "🗂️ [DELETION PROCESSOR] Démarrage du traitement des suppressions programmées",
  );
...
async function checkPendingDeletions() {
  ...
  try {
    ...
    console.log(
      "💡 Suggestion: Ajouter une route GET /api/account/delete/pending pour le monitoring",
    );
  } catch (error) {
    console.warn(
      "⚠️ Erreur lors de la vérification des suppressions en attente:",
      error.message,
    );
  }
}
