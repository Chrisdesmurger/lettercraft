...
try {
  ...
  if (fs.existsSync(envPath)) {
    ...
    supabaseServiceKey = envVars.SUPABASE_SERVICE_ROLE_KEY || envVars.SUPABASE_SERVICE_KEY || envVars.SUPABASE_SECRET_KEY;
    supabaseAnonKey = envVars.NEXT_PUBLIC_SUPABASE_ANON_KEY;
  }
} catch (error) {
  console.warn('⚠️  Impossible de lire .env.local, utilisation des variables système');
}

// Fallback sur les variables d'environnement système
supabaseUrl = supabaseUrl || process.env.NEXT_PUBLIC_SUPABASE_URL;
supabaseServiceKey = supabaseServiceKey || process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_KEY || process.env.SUPABASE_SECRET_KEY;
supabaseAnonKey = supabaseAnonKey || process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;

if (!supabaseUrl || !supabaseServiceKey || !supabaseAnonKey) {
  console.error('❌ Variables d\'environnement Supabase manquantes');
  console.error('Requis: NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_KEY, NEXT_PUBLIC_SUPABASE_ANON_KEY');
  process.exit(1);
}
...
async function checkRLSStatus() {
  ...
  try {
    ...
    data.forEach(table => {
      ...
      const policyStatus = policyCount > 0 ? `${policyCount} policies` : '⚠️  Aucune policy';
      
      console.log(`${status} ${table.table_name.padEnd(30)} RLS: ${table.rls_enabled ? 'ON' : 'OFF'} | ${policyStatus}`);
...
async function testDataIsolation(users) {
  ...
  try {
    ...
    await client1.auth.signInWithPassword({ email: user1.email, password: user1.password });
    await client2.auth.signInWithPassword({ email: user2.email, password: user2.password });
    ...
    if (selectError) {
      ...
      console.error('❌ SÉCURITÉ COMPROMISE: user2 peut voir des profils non autorisés');
      allTestsPassed = false;
    } else {
    ...
    const { error: cvError } = await client1
    ...
    if (user2CVs && user2CVs.length > 0) {
      console.error('❌ SÉCURITÉ COMPROMISE: user2 peut voir des CVs non autorisés');
      allTestsPassed = false;
    } else {
  ...
  for (const table of TABLES_TO_TEST) {
    try {
      ...
      if (error) {
        ...
        console.error(`❌ ${table}: SÉCURITÉ COMPROMISE - Accès anonyme autorisé!`);
        allTestsPassed = false;
      } else {
