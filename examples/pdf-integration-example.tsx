...
export function LetterPreviewWithTemplates({ data }: { data: any }) {
  ...
  const letterData: LetterData = {
    ...
    candidateName:
      data.candidateProfile?.first_name +
      " " +
      data.candidateProfile?.last_name,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="letter-preview">{data.generatedLetter}</div>

      {/* Nouveau système d'export avec modèles */}
      <PdfExportControls letterData={letterData} fileName={fileName} />
    </div>
  );
}
...
export function LetterCardWithTemplates({ letter }: { letter: any }) {
  const letterData: LetterData = {
    ...
    candidateName:
      letter.candidates_profile?.first_name +
      " " +
      letter.candidates_profile?.last_name,
...
export async function generateCustomPdf() {
  ...
  try {
    await generateLetterPdfWithTemplate(letterData, "ma-lettre-personnalisee", {
