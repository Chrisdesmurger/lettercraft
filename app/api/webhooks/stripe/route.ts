...
async function upsertStripeSubscription(
  ...
  try {
    console.log(
      `🔍 [upsertStripeSubscription] Starting upsert for customer ${customerId}`,
    );
    console.log(
      `📊 [upsertStripeSubscription] Subscription data:`,
      subscriptionData,
    );
    ...
    const { data: userByCustomerId, error: customerError } = await supabaseAdmin
      .from("user_profiles")
      .select(
        "user_id, first_name, last_name, stripe_customer_id, stripe_subscription_id, subscription_tier, language",
      )
      .eq("stripe_customer_id", customerId)
      .single();

    console.log(`🔍 [upsertStripeSubscription] User by customer ID:`, {
      userByCustomerId,
      customerError,
    });
    ...
    if (!foundUser) {
      console.log(
        `🔍 User not found by customer ID ${customerId}, trying by email...`,
      );
      ...
      try {
        ...
        if (customer.email) {
          ...
          const userByEmail = authUser.users?.find(
          ...
          if (userByEmail) {
            ...
            console.log(`📧 [upsertStripeSubscription] User by email:`, {
              userByEmail: userByEmail.email,
              userProfile,
            });
            ...
            if (userProfile) {
              ...
              console.log(
                `🔗 Linked customer ${customerId} to user ${foundUser.id}`,
              );
            }
          }
        }
    ...
    console.log(
      `💾 [upsertStripeSubscription] Upserting subscription for user ${foundUser.id}`,
    );
    ...
    console.log(`💾 [upsertStripeSubscription] Upsert result:`, {
      upsertError,
    });

    if (upsertError) {
      console.error(
        `❌ Error upserting subscription for user ${foundUser.id}:`,
        upsertError,
      );
      return false;
    }
    ...
    console.log(
      `✅ Successfully upserted subscription for user ${foundUser.id} (${foundUser.first_name} ${foundUser.last_name})`,
    );
    ...
    console.log(
      `💾 [SUBSCRIPTION STATE] User had previous subscription: ${hadPreviousSubscription}`,
    );

    // Stripe IDs sync is now handled automatically by the subscription trigger
    console.log(
      `🔄 [AUTO SYNC] Stripe IDs will be synced by trigger for user ${foundUser.id}`,
    );
    ...
    try {
      ...
      console.log(
        `🔄 Contact Brevo synchronisé pour l'utilisateur ${foundUser.id}`,
      );
    } catch (syncError) {
      console.warn(
        "Erreur synchronisation contact Brevo après mise à jour abonnement:",
        syncError,
      );
    ...
    console.log(
      `📧 [EMAIL CHECK] Status: ${subscriptionData.status}, CancelAtPeriodEnd: ${subscriptionData.cancel_at_period_end}, User: ${foundUser.email || "email manquant"}`,
    );
    ...
    if (
      ...
      try {
        if (!foundUser.email) {
          console.warn(
            `⚠️ [CANCELLATION EMAIL] Email manquant pour l'utilisateur ${foundUser.id}`,
          );
          return true; // Continue without sending email
        }

        console.log(
          `📧 [CANCELLATION EMAIL] Envoi email d'annulation à ${foundUser.email}`,
        );
        ...
        const emailResult =
          await brevoEmailService.sendSubscriptionCancelledEmail(
            ...
            subscriptionData.current_period_end ||
              new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(), // fin de période ou +30 jours
            userLanguage,
          );

        console.log(
          `📧 [CANCELLATION RESULT] Email d'annulation envoyé: ${emailResult}`,
        );
      } catch (emailError) {
        console.error(
          "❌ [CANCELLATION EMAIL ERROR] Erreur lors de l'envoi de l'email d'annulation:",
          emailError,
        );
      }
    }
    // Envoyer l'email de confirmation d'abonnement premium SEULEMENT pour les nouveaux abonnements actifs
      ...
      try {
        if (!foundUser.email) {
          console.warn(
            `⚠️ [EMAIL SENDING] Email manquant pour l'utilisateur ${foundUser.id}`,
          );
          return true; // Continue without sending email
        }

        console.log(
          `📧 [EMAIL SENDING] Tentative d'envoi à ${foundUser.email}, nom: ${userName}, langue: ${userLanguage}`,
        );
        ...
        console.log(
          `📧 Email de confirmation d'abonnement premium envoyé à ${foundUser.email} (nouvel abonnement)`,
        );
      } catch (emailError) {
        // Ne pas faire échouer le webhook si l'email échoue
        console.error(
          "❌ [EMAIL ERROR] Erreur lors de l'envoi de l'email de confirmation d'abonnement:",
          emailError,
        );
      }
    } else {
      console.log(
        `📧 [EMAIL SKIP] Email ignoré - Status: ${subscriptionData.status}, CancelAtPeriodEnd: ${subscriptionData.cancel_at_period_end}, HadPreviousSubscription: ${hadPreviousSubscription}`,
      );
    }

    console.log(
      `🎯 Subscription ${subscriptionData.stripe_subscription_id} status: ${subscriptionData.status}`,
    );
...
async function upsertStripeInvoice(
  ...
  try {
    console.log(
      `💰 [upsertStripeInvoice] Starting upsert for customer ${customerId}`,
    );
    ...
    console.log(`💰 [upsertStripeInvoice] User by customer ID:`, {
      userByCustomerId,
      customerError,
    });
      ...
      try {
        ...
        if (customer.email) {
          ...
          if (userByEmail) {
            ...
            console.log(`💰 [upsertStripeInvoice] User by email:`, {
              userByEmail: userByEmail.email,
              userProfile,
            });
    ...
    console.log(
      `💾 [upsertStripeInvoice] Upserting invoice for user ${foundUser.id}`,
    );
    ...
    const { error: upsertError } = await supabaseAdmin.rpc(
      "upsert_stripe_invoice",
      {
        ...
        p_collection_method:
          invoiceData.collection_method || "charge_automatically",
    ...
    if (upsertError) {
      console.error(
        `❌ Error upserting invoice for user ${foundUser.id}:`,
        upsertError,
      );
      return false;
    }
    ...
    console.log(
      `✅ Successfully upserted invoice ${invoiceData.stripe_invoice_id} for user ${foundUser.id}`,
    );
    console.log(
      `💰 Invoice amount: ${invoiceData.amount_due / 100} ${invoiceData.currency.toUpperCase()}`,
    );

    // Envoyer un email pour les factures payées (confirmation de paiement)
    console.log(
      `💰 [INVOICE EMAIL CHECK] Status: ${invoiceData.status}, Description: ${invoiceData.description}`,
    );
    ...
    if (invoiceData.status === "paid") {
      try {
        ...
        const isSubscriptionInvoice =
          ...
          invoiceData.description.toLowerCase().includes("subscription");
        console.log(
          `💰 [INVOICE EMAIL] IsSubscriptionInvoice: ${isSubscriptionInvoice}`,
        );

        if (isSubscriptionInvoice) {
          if (!foundUser.email) {
            console.warn(
              `⚠️ [INVOICE EMAIL] Email manquant pour l'utilisateur ${foundUser.id}`,
            );
            return true; // Continue without sending email
          }

          console.log(
            `💰 [INVOICE EMAIL SENDING] Tentative d'envoi à ${foundUser.email}`,
          );
          ...
          console.log(
            `📧 Email de confirmation premium envoyé à ${foundUser.email} (via facture payée)`,
          );
        }
      } catch (emailError) {
        // Ne pas faire échouer le webhook si l'email échoue
        console.error(
          "❌ [INVOICE EMAIL ERROR] Erreur lors de l'envoi de l'email de confirmation de facture:",
          emailError,
        );
      }
    }
    ...
    if (
      ...
      invoiceData.attempt_count &&
      invoiceData.attempt_count > 0
    ) {
      try {
        if (!foundUser.email) {
          console.warn(
            `⚠️ [PAYMENT FAILED EMAIL] Email manquant pour l'utilisateur ${foundUser.id}`,
          );
          return true; // Continue without sending email
        }
...
async function handleInvoicePaymentSucceeded(invoice: Stripe.Invoice) {
  console.log(
    "🎯 [WEBHOOK] Processing successful payment for invoice:",
    invoice.id,
  );
  ...
  console.log(
    "📧 [INVOICE] Email d'abonnement géré par la logique subscription, pas par invoice",
  );
...
async function handleInvoicePaymentFailed(invoice: Stripe.Invoice) {
  console.log(
    "🎯 [WEBHOOK] Processing failed payment for invoice:",
    invoice.id,
  );
...
async function handleCustomerSubscriptionTrialWillEnd(
  ...
  const trialEndDate = trialEnd ? new Date(trialEnd * 1000) : "unknown";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(
      `🎯 [WEBHOOK] Processing webhook event: ${event.type} (ID: ${event.id})`,
    );
    console.log(
    ...
    const processingPromise = (async () => {
      ...
      switch (event.type) {
        case "customer.subscription.created":
          success = await handleCustomerSubscriptionCreated(
            event.data.object as Stripe.Subscription,
          );
          break;

        case "customer.subscription.updated":
          success = await handleCustomerSubscriptionUpdated(
            event.data.object as Stripe.Subscription,
          );
          break;

        case "customer.subscription.deleted":
          success = await handleCustomerSubscriptionDeleted(
            event.data.object as Stripe.Subscription,
          );
          break;

        case "invoice.payment_succeeded":
          success = await handleInvoicePaymentSucceeded(
            event.data.object as Stripe.Invoice,
          );
          break;

        case "invoice.payment_failed":
          success = await handleInvoicePaymentFailed(
            event.data.object as Stripe.Invoice,
          );
          break;

        case "invoice.created":
          success = await handleInvoiceCreated(
            event.data.object as Stripe.Invoice,
          );
          break;

        case "invoice.updated":
          success = await handleInvoiceUpdated(
            event.data.object as Stripe.Invoice,
          );
          break;

        case "customer.subscription.trial_will_end":
          success = await handleCustomerSubscriptionTrialWillEnd(
            event.data.object as Stripe.Subscription,
          );
          break;

        case "customer.updated":
          success = await handleCustomerUpdated(
    ...
    const result = (await Promise.race([
      processingPromise,
      timeoutPromise,
    ])) as { success: boolean; eventType: string; eventId: string };

    const processingTime = Date.now() - startTime;
    console.log(
      `Event ${result.eventType} (${result.eventId}) processed in ${processingTime}ms - Success: ${result.success}`,
    );

    if (!result.success) {
      console.error(
        `Failed to process event ${result.eventType} (${result.eventId})`,
      );
      return NextResponse.json(
