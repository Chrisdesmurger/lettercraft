...
const validationSchemas = {
  ...
  bulk: {
    ...
    userIds: { required: true, type: 'array', minItems: 1, maxItems: 50, itemType: 'string' }
  },
  sync: {
  ...
  'update-lists': {
    ...
    email: { required: true, type: 'string', format: 'email' },
    listIds: { required: true, type: 'array', minItems: 1, maxItems: 10, itemType: 'number' }
  },
  'sync-all-lists': {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const isInternalCall = internalSecret === (process.env.INTERNAL_API_SECRET || 'lettercraft-internal-secret-2025')
    ...
    const isAdminAction = ['bulk', 'create-missing', 'sync-all-lists', 'delete'].includes(action)
    ...
    if (isInternalCall) {
      ...
      const securityConfig = {
        ...
        validationSchema: validationSchemas[action as keyof typeof validationSchemas]
      }
    ...
    if (!security.allowed) {
      console.warn(`🚫 Accès refusé pour l'action '${action}':`, security.error?.message)
    ...
    const { userId, userIds, email, firstName, lastName, language, listIds } = validatedData
    ...
    if (!isInternalCall) {
      ...
      if (!permissionCheck.allowed) {
        console.warn(`🚫 Permission refusée pour '${action}':`, permissionCheck.reason)
        return NextResponse.json(
    ...
    if (isDangerousAction || isAdminAction) {
      ...
      console.warn(`⚠️ Action '${action}' exécutée par ${source} (${context?.userId || 'internal'})`)
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`🔍 Récupération contact Brevo: ${email} (par ${context?.email})`)
