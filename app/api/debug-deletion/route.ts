...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `Auth user:`,
      authUser?.user ? "EXISTS" : "NOT FOUND",
      authError,
    );
    ...
    const relationsData = {
      generated_letters:
        relations[0].status === "fulfilled"
          ? relations[0].value.count
          : "ERROR",
      saved_letters:
        relations[1].status === "fulfilled"
          ? relations[1].value.count
          : "ERROR",
      user_quotas:
        relations[2].status === "fulfilled"
          ? relations[2].value.count
          : "ERROR",
      candidates_profile:
        relations[3].status === "fulfilled"
          ? relations[3].value.count
          : "ERROR",
      job_offers:
        relations[4].status === "fulfilled"
          ? relations[4].value.count
          : "ERROR",
      stripe_subscriptions:
        relations[5].status === "fulfilled"
          ? relations[5].value.count
          : "ERROR",
      stripe_invoices:
        relations[6].status === "fulfilled"
          ? relations[6].value.count
          : "ERROR",
    };
    ...
    try {
      ...
      const { data: hardDeleteResult, error: hardDeleteError } =
        await supabaseAdmin.rpc("execute_hard_delete_user", {
          p_user_id: userId,
        });
