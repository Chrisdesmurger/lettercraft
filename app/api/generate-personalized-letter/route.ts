...
import {
  parseLetterResponse,
  cleanLetterSections,
  formatLetterSections,
  validateContentCleanliness,
} from "@/lib/letter-sections";
import { getOpenAIConfig } from "@/lib/openai-config";
import {
  TONE_GUIDELINES,
  type ToneKey,
  getToneDirectives,
} from "@/lib/tone-guidelines";

const openai = new OpenAI({
  apiKey: process.env.OPENAI_API_KEY,
});

async function generatePersonalizedLetterHandler(
  request: NextRequest,
  userId: string,
) {
  try {
    const { jobOffer, questionnaireResponse, cvData, settings } =
      await request.json();
    ...
    const context = {
      ...
      settings: {
        ...
        tone: settings?.toneKey || settings?.tone || "professionnel",
        toneKey: settings?.toneKey || "professionnel",
        customTone: settings?.customTone || "",
        length: settings?.length || "moyen",
      },
    };

    // Obtenir les directives de ton
    const toneDirectives = getToneDirectives(
      context.settings.toneKey as ToneKey,
      context.settings.customTone,
    );
...
2. Adopt this specific tone: ${toneDirectives} - Length: ${context.settings.length}
...
BANNED PHRASES: "I am writing to", "with great interest", "esteemed company", "I would welcome", "please find attached", "thank you for your consideration"`,
              ...
              bodyInstructions:
                "Write a compelling and deeply personalized cover letter body that tells a specific story using concrete details from the candidate's experience, demonstrates clear measurable value, and shows authentic enthusiasm with specific references to the company and role",
            };
          case "es":
            return {
              language: "Spanish",
...
2. Adopta este tono específico: ${toneDirectives} - Longitud: ${context.settings.length}
...
- Conclusión sólida con llamada a la acción segura`,
              ...
              bodyInstructions:
                "Escribe un cuerpo de carta convincente y profundamente personalizado que cuente una historia específica usando detalles concretos de la experiencia del candidato, demuestre valor medible claro, y muestre entusiasmo auténtico con referencias específicas a la empresa y el puesto",
            };
          default: // français
            return {
              language: "French",
...
2. Adopte ce ton spécifique: ${toneDirectives} - Longueur: ${context.settings.length}
...
PHRASES BANNIES: "C'est avec grand intérêt", "votre estimée entreprise", "Je me permets", "J'aimerais avoir l'opportunité", "Veuillez trouver ci-joint", "Je vous remercie de l'attention"`,
              ...
              bodyInstructions:
                "Rédige un corps de lettre convaincant et profondément personnalisé qui raconte une histoire spécifique en utilisant des détails concrets de l'expérience du candidat, démontre une valeur mesurable claire, et montre un enthousiasme authentique avec des références spécifiques à l'entreprise et au poste",
            };
        }
      };
...
BODY: [votre corps de lettre de 300 mots intégrant TOUTES les obligations spécifiques listées ci-dessus]`;
      ...
      messages: [
        {
          role: "system",
          content:
            "Tu es un expert en rédaction de lettres de motivation professionnelles. Tu DOIS impérativement respecter TOUTES les instructions données. Utilise OBLIGATOIREMENT les phrases exactes fournies dans le questionnaire. Réponds toujours avec la structure demandée utilisant les balises SUBJECT:, GREETING:, et BODY:. Commence directement par SUBJECT: sans aucun texte avant.",
        },
        {
          role: "user",
    ...
    if (!validation.isClean) {
      console.warn(
        "Generated content contains personal information:",
        validation.issues,
      );
    }
...
Genera únicamente el código HTML completo, sin comentarios.`,
      ...
      messages: [
        {
          ...
          content:
            htmlInstructions[
              context.settings.language as keyof typeof htmlInstructions
            ] || htmlInstructions.fr,
        },
        {
          role: "user",
    ...
    const htmlContent =
      htmlCompletion.choices[0].message.content || letterContent;
      ...
      metadata: {
        ...
        context_used: {
          ...
          experience_highlighted:
            context.responses.experience_highlight.experience_title,
