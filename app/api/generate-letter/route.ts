...
async function generateLetterHandler(request: NextRequest, userId: string) {
  try {
    ...
    if (!isValidToneKey(toneKey)) {
      return NextResponse.json({ error: "Ton non valide" }, { status: 400 });
    }
    ...
    if (!toneValidation.valid) {
      return NextResponse.json(
        {
          error: "Paramètres de ton invalides",
    ...
    const sanitizedToneCustom = toneCustom
      ? sanitizeToneText(toneCustom)
      : undefined;
      ...
      messages: [
        {
          role: "system",
          content:
            "Tu es un expert en rédaction de lettres de motivation professionnelles. Réponds toujours avec la structure demandée utilisant les balises SUBJECT:, GREETING:, et BODY:.",
        },
        {
          role: "user",
          content: prompt,
