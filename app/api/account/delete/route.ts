...
async function cancelStripeSubscriptionWithRefund(
  customerId: string,
  ...
  try {
    ...
    if (subscription.status !== "active") {
      console.log(
        `⚠️ Subscription ${subscriptionId} is not active (status: ${subscription.status})`,
      );
    ...
    if (invoices.data.length === 0) {
      console.log(
        `ℹ️ No paid invoices found for subscription ${subscriptionId}`,
      );
    ...
    if (invoiceAge > maxRefundAge) {
      console.log(
        `⏰ Latest invoice is too old for refund (${Math.floor(invoiceAge / (24 * 60 * 60 * 1000))} days)`,
      );
    ...
    if (chargeId) {
      ...
      if (refundAmount > 0) {
        ...
        console.log(
          `💰 Refund processed: ${refundAmount / 100} ${latestInvoice.currency} (${Math.round(unusedRatio * 100)}% unused)`,
        );
...
async function handleRequestDeletion(request: NextRequest) {
  ...
  const { data: deletionRequest, error: requestError } =
    await supabaseAdmin.rpc("create_account_deletion_request", {
      p_user_id: userId,
  ...
  if (confirmationEmail) {
    try {
      ...
      console.log(
        `📧 Email de confirmation de suppression envoyé à ${userEmail}`,
      );
    } catch (emailError) {
      console.warn(
...
async function handleConfirmDeletion(request: NextRequest) {
  ...
  console.log(
    `🎯 Confirmation de suppression reçue - annulation immédiate de l'abonnement pour l'utilisateur ${userId}`,
  );
  ...
  try {
    ...
    const { data: userProfile, error: profileError } = await supabaseAdmin
      .from("user_profiles")
      .select(
        "first_name, last_name, stripe_customer_id, stripe_subscription_id, subscription_tier, language",
      )
      .eq("user_id", userId)
      .single();
    ...
    if (authUser?.user && !profileError && userProfile) {
      ...
      console.log(
        `🔍 Utilisateur trouvé: ${userEmail}, Stripe Customer ID: ${userProfile.stripe_customer_id}, Subscription ID: ${userProfile.stripe_subscription_id}`,
      );
      ...
      if (
        ...
        console.log(
          `💳 Annulation immédiate de l'abonnement Stripe pour l'utilisateur ${userId}`,
        );
        ...
        try {
          ...
          if (
            ...
            console.log(
              `✅ Abonnement Stripe ${userProfile.stripe_subscription_id} annulé avec succès`,
            );
            console.log(
              `📅 Date d'annulation: ${new Date(cancelledSubscription.canceled_at! * 1000).toISOString()}`,
            );
            ...
            try {
              ...
              console.log(
                `📧 Email d'annulation d'abonnement envoyé à ${userEmail}`,
              );
            } catch (emailError) {
              console.warn(
                "Erreur lors de l'envoi de l'email d'annulation d'abonnement:",
                emailError,
              );
            }
            ...
            try {
              await fetch(
                `${process.env.NEXT_PUBLIC_APP_URL}/api/sync-contact`,
                {
                  method: "POST",
                  headers: {
              ...
              console.log(
                `🔄 Contact Brevo synchronisé après annulation d'abonnement pour l'utilisateur ${userId}`,
              );
            } catch (syncError) {
              console.warn(
                "Erreur synchronisation contact Brevo après annulation abonnement:",
                syncError,
              );
            }
          } else {
            console.log(
              `ℹ️ L'abonnement ${userProfile.stripe_subscription_id} n'est pas actif ou déjà annulé (status: ${subscription.status}, cancel_at_period_end: ${subscription.cancel_at_period_end})`,
            );
          }
        } catch (stripeError) {
          console.error(
            `❌ Erreur lors de l'annulation de l'abonnement Stripe pour l'utilisateur ${userId}:`,
            stripeError,
          );
        ...
        console.log(
          `ℹ️ Aucun abonnement Stripe actif trouvé pour l'utilisateur ${userId}`,
        );
      }
    } else {
      console.warn(
        `⚠️ Impossible de récupérer le profil utilisateur ${userId}:`,
        profileError,
      );
    }
  } catch (error) {
    console.error(
      `❌ Erreur lors du traitement de l'annulation d'abonnement pour l'utilisateur ${userId}:`,
      error,
    );
  ...
  return NextResponse.json({
    success: true,
    message:
      "Suppression confirmée avec succès. Votre abonnement a été annulé immédiatement.",
    userId,
    scheduledDeletionAt,
...
async function handleCancelDeletion(request: NextRequest) {
  ...
  // Annuler la demande
  const { data: cancelled } = await supabaseAdmin.rpc(
...
async function executeScheduledDeletions() {
  ...
  for (const deletion of pendingDeletions) {
    try {
      ...
      console.error(
        `Error executing deletion for user ${deletion.user_id}:`,
        error,
      );
      failed++;
    }
  ...
  console.log(
    `✅ Deletion batch completed: ${executed} executed, ${failed} failed`,
  );
...
async function executeUserDeletionInternal(
  ...
  const { data: userProfile, error: profileError } = await supabaseAdmin
    .from("user_profiles")
    .select(
      "first_name, last_name, stripe_customer_id, stripe_subscription_id, language",
    )
    .eq("user_id", userId)
    .single();
  ...
  const userEmail = authUser.user.email!;
  console.log(
    `🗑️ Executing ${deletionType} deletion for user ${userId} (${userEmail})`,
  );
  ...
  try {
    ...
    if (deletionType === "soft") {
      const { data: result } = await supabaseAdmin.rpc(
        "execute_soft_delete_user",
        { p_user_id: userId },
      );
      deletionResult = result;
    } else {
      const { data: result } = await supabaseAdmin.rpc(
        "execute_hard_delete_user",
        { p_user_id: userId },
      );
      deletionResult = result;
    }
