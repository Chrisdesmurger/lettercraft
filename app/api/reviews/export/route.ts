...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!hasAccess) {
      return NextResponse.json(
        {
          error: "Accès non autorisé à l'export",
    ...
    const { data: rows, error: exportError } = await supabase.rpc(
      "export_reviews_csv",
      {
    ...
    console.log(
      `CSV export: user=${user.id}, rows=${rows?.length || 0}, period=${daysBack}days`,
    );
