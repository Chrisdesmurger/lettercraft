...
export async function POST(request: NextRequest) {
  try {
    ...
    if (adminSecret !== expectedSecret) {
      console.warn(
        `🚨 [SECURITY] Unauthorized cleanup attempt from IP: ${request.headers.get("x-forwarded-for") || "unknown"}`,
      );
...
async function cleanupExpiredRequests() {
  ...
  try {
    ...
    console.log(
      `✅ Cleanup completed: ${cleanedCount} expired requests cleaned`,
    );
...
async function ensureGenericUserExists(): Promise<string | null> {
  ...
  try {
    ...
    if (!listError && users?.users) {
      const existingGenericUser = users.users.find(
        (user) => user.email === genericEmail,
      );

      if (existingGenericUser) {
        console.log(
          `✅ Found existing generic user with different ID: ${existingGenericUser.id}`,
        );
        ...
        if (profileError) {
          console.warn(
            "⚠️ Warning: Could not ensure generic user profile:",
    ...
    if (createError) {
      ...
      if (createError.code === "email_exists") {
        ...
        const conflictUser = retryUsers?.users?.find(
          (user) => user.email === genericEmail,
        );

        if (conflictUser) {
          console.log(
            `✅ Using existing user with email conflict: ${conflictUser.id}`,
          );
    ...
    if (actualUserId) {
      ...
      if (profileError) {
        console.warn(
          "⚠️ Warning: Could not create generic user profile:",
          profileError.message,
        );
    ...
    for (const deletion of pendingDeletions) {
      try {
        ...
        const { data: userProfile } = await supabaseAdmin
          .from("user_profiles")
          .select(
            "first_name, last_name, stripe_customer_id, stripe_subscription_id",
          )
        ...
        if (!authUser?.user) {
          console.warn(
            `⚠️ User ${deletion.user_id} not found in auth, skipping...`,
          );
          continue;
        }
        ...
        console.log(
          `🗑️ Processing deletion for user ${deletion.user_id} (${userEmail})`,
        );
        ...
        if (deletionResult) {
          ...
          console.log(
            `✅ Successfully deleted user ${deletion.user_id} (${deletion.deletion_type})`,
          );
        } else {
          failed++;
          results.push({
