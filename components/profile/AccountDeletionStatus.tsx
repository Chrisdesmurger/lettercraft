    ...
    >
      <div className="flex items-start space-x-4">
        ...
        <div className="flex-1">
          ...
          >
            {isConfirmed
              ? t("account.deletion.confirmed")
              : t("account.deletion.pending")}
          </h3>
          ...
          >
            <p className="mb-2">
              {t("account.deletion.willBeDeleted", {
                type:
                  deletionRequest.deletion_type === "hard"
            ...
            <p className="font-medium">
              {timeRemaining.urgent ? (
                <span className="text-red-800">
                  ⚠️ {t("account.deletion.urgentWarning")}
                </span>
              ) : (
                <>
                  {t("account.deletion.scheduledIn", {
            ...
            {!isConfirmed && (
              <p className="mt-2 text-xs">
                <strong>{t("account.deletion.actionRequired")}:</strong>{" "}
                {t("account.deletion.checkEmail")}
              </p>
            )}
          </div>
          ...
          {deletionRequest.reason && (
            <div className="mt-3 p-3 bg-white bg-opacity-50 rounded border">
              <div className="text-xs font-medium text-gray-600 mb-1">
                {t("account.deletion.reasonGiven")}:
              </div>
      ...
      {timeRemaining.urgent && (
        <div className="mt-4 p-3 bg-red-100 border border-red-200 rounded">
          <div className="flex items-center text-red-800 text-sm">
            ...
            <div>
              <strong>{t("account.deletion.lastChance")}</strong>{" "}
              {t("account.deletion.urgentMessage")}
            </div>
          </div>
        </div>
      )}
