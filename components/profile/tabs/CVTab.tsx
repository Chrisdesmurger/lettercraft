...
export default function CVTab() {
  const { t } = useI18n();
  const { cvs, loading, error, setActiveCV, deleteCV, downloadCV } =
    useUserCVs();
    ...
    <div className="space-y-6">
      ...
      {canUpload ? (
          ...
          <p className="text-sm text-gray-500 mt-2">
            ...
            {subscriptionTier === "premium" && (
              <span className="ml-2 text-purple-600 font-medium flex items-center gap-1">
                <Crown className="w-3 h-3" />({t("cv.premiumLimit")})
              </span>
            )}
          </p>
        </div>
        ...
        <div className="border-2 border-dashed border-gray-200 rounded-lg p-8 text-center bg-gray-50">
          ...
          <p className="text-sm text-gray-400 mb-3">{t("cv.deleteToAddNew")}</p>
          {subscriptionTier === "free" && (
