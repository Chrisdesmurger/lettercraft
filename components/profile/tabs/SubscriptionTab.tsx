  ...
  const plans = [
    {
      ...
      features: [
        ...
        {
          name: t("subscription.features.multilingualGeneration"),
          included: false,
        },
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-blue-700">
                {quota && !quotaLoading
                  ? t("subscription.remainingLetters", {
                      count: quota.remaining_letters.toString(),
                    })
                  : t("common.loading")}
              </p>
            </div>
      ...
      {currentPlan === "premium" && subscriptionStatus && (
        <div
          className={`border rounded-lg p-4 ${subscriptionStatus.is_canceled ? "bg-orange-50 border-orange-200" : "bg-green-50 border-green-200"}`}
        >
          <div className="flex items-start justify-between">
            <div>
              <p
                className={`font-medium ${subscriptionStatus.is_canceled ? "text-orange-900" : "text-green-900"}`}
              >
                {subscriptionStatus.is_canceled
                  ? t("subscription.canceledPremium")
                  : t("subscription.activePremium")}
              </p>
              <p
                className={`text-sm ${subscriptionStatus.is_canceled ? "text-orange-700" : "text-green-700"}`}
              >
                {subscriptionStatus.is_canceled ? (
                  ...
                  t("subscription.activeUntil", {
                    date: subscriptionStatus.expiration_date
                      ? new Date(
                          subscriptionStatus.expiration_date,
                        ).toLocaleDateString(locale)
                      : "unknown",
        ...
        {plans.map((plan) => (
          <div
            ...
            className={`relative border rounded-xl p-6 ${
              plan.popular ? "border-orange-400 shadow-lg" : "border-gray-200"
              ...
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">
                    {t("subscription.table.date")}
                  </th>
                  <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">
                    {t("subscription.table.description")}
                  </th>
                  <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">
                ...
                {!invoicesLoading && invoices.length === 0 ? (
                  ...
                  invoices.map((invoice) => (
                    <tr key={invoice.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm">
                        <span
                          className={`font-medium ${getInvoiceStatusColor(invoice.status)}`}
                        >
                        ...
                        <div className="flex items-center space-x-2">
                          {invoice.invoice_pdf || invoice.hosted_invoice_url ? (
                            ...
                            <span className="text-gray-400 text-sm">
                              {t("subscription.invoice.notAvailable")}
                            </span>
                          )}
          ...
          {invoices.length > 0 && (
            <div className="mt-4 text-center">
              ...
              >
                {invoicesLoading
                  ? t("common.refreshing")
                  : t("subscription.invoice.refresh")}
              </button>
            </div>
          )}
        </div>
