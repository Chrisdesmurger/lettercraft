...
import { FeedbackCategory, CreateReviewData, REVIEW_CONSTRAINTS } from "@/types/reviews"
...
export const ReviewModal = React.forwardRef<HTMLDivElement, ReviewModalProps>(
  ({ isOpen, onClose, onSubmit, letterId, isSubmitting = false }, ref) => {
    ...
    const [isLocalSubmitting, setIsLocalSubmitting] = React.useState<boolean>(false)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        toast.error(t('reviews.submitError') || 'Erreur lors de l\'envoi de votre avis')
      } finally {
        setIsLocalSubmitting(false)
      }
    }
    ...
    const remainingChars = REVIEW_CONSTRAINTS.MAX_FEEDBACK_LENGTH - feedback.length
    ...
    return (
      <Dialog open={isOpen} onOpenChange={() => !isSubmitting && onClose()}>
        ...
        >
          <DialogHeader>
            <DialogTitle>
              {t('reviews.modal.title') || 'Comment évaluez-vous cette lettre ?'}
            </DialogTitle>
            <DialogDescription>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              {rating > 0 && (
                <p className="text-xs text-center text-muted-foreground">
                  {rating === 1 && (t('reviews.rating.labels.1') || 'Très insatisfait')}
                  {rating === 2 && (t('reviews.rating.labels.2') || 'Insatisfait')}
                  {rating === 3 && (t('reviews.rating.labels.3') || 'Neutre')}
                  {rating === 4 && (t('reviews.rating.labels.4') || 'Satisfait')}
                  {rating === 5 && (t('reviews.rating.labels.5') || 'Très satisfait')}
                </p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="feedback"
                placeholder={t('reviews.feedback.placeholder') || 
              ...
              <div className="flex justify-between text-xs text-muted-foreground">
                <span>
                  {t('reviews.feedback.optional') || 'Optionnel'}
                </span>
                <span className={remainingChars < 20 ? 'text-orange-500' : ''}>
                  {t('reviews.feedback.charactersRemaining', { count: remainingChars }) || 
            ...
            <div className="space-y-2">
              <Label className="text-sm font-medium">
                {t('reviews.categories.label') || 'Aspects à améliorer (optionnel)'}
              </Label>
              <CategorySelector
