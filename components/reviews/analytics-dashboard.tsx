...
export const AnalyticsDashboard = React.forwardRef<HTMLDivElement, AnalyticsDashboardProps>(
  ({ className, defaultPeriodDays = 30 }, ref) => {
    ...
    const exportCSV = async () => {
      ...
      try {
        const response = await fetch(`/api/reviews/export?days=${state.periodDays}`)
      ...
      <div ref={ref} className={cn("space-y-6", className)}>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {isLoading ? '---' : `${data?.participation_rate?.toFixed(1) || 0}%`}
              </div>
              <p className="text-xs text-muted-foreground">
                {isLoading ? '' : `${data?.total_users_with_reviews || 0}/${data?.total_users_with_letters || 0} utilisateurs`}
              </p>
            </CardContent>
          </Card>
            ...
            {isLoading ? (
              ...
              <div className="space-y-2">
                {[5, 4, 3, 2, 1].map((rating) => {
                  const count = data?.rating_distribution?.[rating.toString()] || 0
