...
export default function PdfExportControls({
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      toast.error(
        `${t("letter.generateError")}: ${error instanceof Error ? error.message : t("auth.unknownError")}`,
      );
    } finally {
      ...
      <CardContent className="p-4">
        <div className="space-y-4">
                  ...
                  <div className="flex items-start space-x-2">
                    ...
                    <div>
                      <p className="font-medium text-blue-700">
                        {t("pdfTemplates.upgradeRequired")}
                      </p>
                      <p className="text-blue-600">
                        {t("pdfTemplates.upgradeMessage")}
                      </p>
                    </div>
                  </div>
          ...
          <div className="text-xs text-muted-foreground bg-muted/50 p-2 rounded">
            <strong>{t("letter.fileName")}:</strong> {fileName}.pdf
            <br />
            <strong>{t("letter.format")}:</strong> A4, {t("letter.highQuality")}
            <br />
            <strong>{t("pdfTemplates.title")}:</strong>{" "}
            {t(`pdfTemplates.templates.${selectedTemplateId}.name`)}
          </div>
        </div>
      </CardContent>
    </Card>
